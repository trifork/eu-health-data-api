...
The choice of approach is **orthogonal to organizational deployment topology**. A FHIR-native server and an XDS system can each be deployed directly, behind a facade, or as part of a national infrastructure. The shared API surface — ITI-67, ITI-68, and the `EehrxfMhdDocumentReference` profile — works for both.
...
- A FHIR-native server MAY also expose the same clinical resources directly via [Resource Access](resource-access.html). Documents and resources are complementary views of the same underlying data.
...
**XDS Proxy** — an MHD translation layer over a legacy XDS/XCA repository. FHIR API calls are translated to XDS transactions against the underlying registry and repository. `DocumentReference` maps directly to XDS `DocumentEntry` per MHD's normative mapping. Existing national XDS investments require no replacement.
...
| | FHIR-Native Server | Document-Centric System |
|---|---|---|
...
[IHE MHD](https://profiles.ihe.net/ITI/MHD/) enables this interoperability: `DocumentReference` maps to XDS `DocumentEntry`, allowing FHIR-native and XDS systems to participate in the same document exchange network.
